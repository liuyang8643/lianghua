...
红线清单：
1. 数据泄露（前视野）：calc_batch 当日价格只能用 panel['open']。绝对禁止以任何形式（含间接、滚动窗口
   不滞后、.get 等）引用 <<FORBIDDEN>>。窗口计算必须保证最近数据点只到 T-1 或 T 日 open。
...
3. 连续分数：必须对【整个 base_valid 全集】（~np.isnan(open) & open>=2 & ~st_mask）连续打分：
