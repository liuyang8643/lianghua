...
回测在 **T 日开盘价** 执行交易，因此因子计算和交易逻辑**禁止使用 T 日开盘之后的任何数据**：

| 数据 | T 日可用？ | 说明 |
|---|---|---|
| `open[T]` | ✅ 可用 | 开盘价在交易时刻已知 |
| `close[T]` | ❌ 泄露 | 收盘价在开盘时未知 |
| `high[T]` | ❌ 泄露 | 最高价在开盘时未知 |
| `low[T]` | ❌ 泄露 | 最低价在开盘时未知 |
| `volume[T]` | ❌ 泄露 | 成交量在开盘时未知 |
| `amount[T]` | ❌ 泄露 | 成交额在开盘时未知 |

**验收时必须逐因子检查 `calc_batch` 的 numpy 窗口计算是否引用到 T 日收盘后数据。** 常见违规模式：
- `sliding_window_view` 窗口若不滞后 1 天，会包含当前日收盘价
- 任何对 `close`/`high`/`low` 在 `trade_idx` 位置的引用（非 open 字段）
- 因子 score 未 shift(1) 直接用于当日选股

检查方法：读取因子文件，跟踪 numpy 数组的时间维度索引偏移，确认最近数据点只到 T-1 日（或 T 日 open）。
