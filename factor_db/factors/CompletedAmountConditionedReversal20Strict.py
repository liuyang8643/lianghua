"""Strict 20-day completed amount-conditioned reversal factor."""
...
class CompletedAmountConditionedReversal20Strict:
    ...
    Row ``T`` uses exactly ``[T-20, T)``.  For each completed row, direction
    is the direct comparison of close with official preClose.  The result is
    the unweighted mean direction minus its RMB-amount-weighted mean.  Flat
    rows contribute zero direction while their positive amount remains in the
    weighted denominator.  Every close, preClose, and amount observation must
    be finite and strictly positive or the affected window is missing.
    """
    ...
    def calc_batch(self, panel: dict) -> np.ndarray:
        ...
        # suffix of the preceding 20-row block plus the already completed
        # prefix of the current block.  Each part is freshly summarized, so a
        # dominant amount leaving a window never requires unstable subtraction.
        ...
        suffix_sign_sum = np.zeros((window + 1, stocks), dtype=np.int16)
        ...
        for block_start in range(window, rows, window):
            ...
            for offset in range(window - 1, -1, -1):
                ...
                daily_direction = np.greater(close_row, pre_close_row).astype(
                    np.int8
                )
                ...
                suffix_sign_sum[offset] = (
                    suffix_sign_sum[offset + 1] + daily_direction
                )
                suffix_count[offset] = suffix_count[offset + 1] + daily_valid
            ...
            for offset in range(block_rows):
                ...
                combined_sign_sum = suffix_sign_sum[offset] + prefix_sign_sum
                combined_count = suffix_count[offset] + prefix_count
                with np.errstate(
                    divide="ignore", invalid="ignore", over="ignore"
                ):
                    unweighted = combined_sign_sum / window
                    weighted = combined_signed / combined_total
                    score = unweighted - weighted
                valid = (
                ...
                prefix_signed = (
                    prefix_signed * old_weight + daily_weight * daily_direction
                )
                prefix_scale = new_scale
                prefix_sign_sum += daily_direction
