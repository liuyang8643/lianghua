"""Causal price-path trend score for the small-cap combination."""
...
class TrendSmallCapCausal:
    """Completed-price stability score; T-day open is used only for legality."""
    ...
    def calc_batch(self, panel: dict) -> np.ndarray:
        ...
        stable = 0.25 * reversal120 + 0.75 * low_vol20
        # Missing trend history is neutral; it must not remove a stock from
        # the independent small-cap/liquidity candidate universe.
        stable = np.nan_to_num(stable, nan=0.5, posinf=0.5, neginf=0.5)
        ...
        return np.where(legal, stable, np.nan).astype(np.float32)
