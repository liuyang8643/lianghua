...
import research_v36_signed_amount_imbalance_ablation as v36


def _fixed_date_strings() -> list[str]:
    all_dates = np.arange(
        np.datetime64(v36.TRAIN_FIRST_DATE, "D"),
        np.datetime64(v36.TRAIN_LAST_DATE, "D") + np.timedelta64(1, "D"),
    )
    positions = np.linspace(
        0,
        len(all_dates) - 1,
        v36.EXPECTED_TRAIN_DAYS,
        dtype=np.int64,
    )
    selected = all_dates[positions]
    assert len(np.unique(selected)) == v36.EXPECTED_TRAIN_DAYS
...
def _valid_kline_payload() -> dict:
    return {
        "000001.SZ": {
            "n": "平安银行",
            "d": [v36.TRAIN_FIRST_DATE, v36.TRAIN_LAST_DATE],
            "o": [9.0, 10.0],
            "h": [9.1, 10.1],
            "l": [8.9, 9.9],
            "c": [9.0, 10.0],
            "a": [900.0, 1000.0],
            "events": [{"date": v36.TRAIN_LAST_DATE, "action": "buy"}],
            "episodes": [
                {
                    "start": v36.TRAIN_LAST_DATE,
                    "end": v36.TRAIN_LAST_DATE,
                    "window_start": v36.TRAIN_FIRST_DATE,
                    "window_end": v36.TRAIN_LAST_DATE,
                }
            ],
        }
    }
...
def _structural_fixture() -> tuple[dict, set[str], list[dict], list[int], np.ndarray]:
    rows = v36.EXPECTED_TRAIN_DAYS + 1
    columns = 30
    shape = (rows, columns)
    arrays = {
        f"_factor_valid_{name}": np.ones(shape, dtype=bool)
        for name in v36.ALL_FACTOR_NAMES
    }
    arrays.update(
        {name: np.ones(shape, dtype=bool) for name in v36.EXPECTED_FILTER_NAMES}
    )
    control_config = {
        "weights": copy.deepcopy(v36.CONTROL_WEIGHTS),
        "filter_factors": {name: True for name in v36.EXPECTED_FILTER_NAMES},
    }
    candidate_config = copy.deepcopy(control_config)
    candidate_config["weights"] = copy.deepcopy(v36.CANDIDATE_WEIGHTS)
    candidates = [
        {
            "role": "control",
            "label": v36.CONTROL_LABEL,
            "individual_config": control_config,
        },
        {
            "role": "candidate",
            "label": v36.CANDIDATE_LABEL,
            "individual_config": candidate_config,
        },
    ]
    return (
        arrays,
        set(v36.ALL_FACTOR_NAMES),
        candidates,
        list(range(v36.EXPECTED_TRAIN_DAYS)),
...
def _valid_source_lock(digest: str = "a" * 64) -> dict:
    return {
        "experiment": v36.EXPERIMENT,
        "status": v36.SOURCE_LOCK_STATUS,
        "sources": {
            name: {
                "path": str(path).replace("\\", "/"),
                "sha256": digest,
            }
            for name, path in v36.NEW_V36_SOURCE_PATHS.items()
        },
    }


def _v35_npz_fields() -> set[str]:
    return {
        "dates",
...
def _v35_control_fields() -> set[str]:
    return {
        "dates",
        "control_daily_returns",
        "control_daily_exposures",
...
def _synthetic_v35_npz_arrays() -> dict[str, np.ndarray]:
    dates = np.asarray(_fixed_date_strings(), dtype="U10")
    days = v36.EXPECTED_TRAIN_DAYS
    return {
        "dates": dates,
...
def _write_synthetic_v35_npz(
    path: Path, arrays: dict[str, np.ndarray]
) -> None:
    assert set(arrays) == _v35_npz_fields()
...
def test_plan_sha_and_execution_semantics_are_strictly_pinned(
    ...
    assert v36._sha256(v36.PLAN_PATH) == (
        "821805617d16b454654de4d83553279c995e617f98e44147241985de7d91f861"
    )
    plan = v36.load_and_validate_plan()
    assert plan["factor"]["class_name"] == v36.NEW_FACTOR_NAME
    assert plan["factor"]["hist_days"] == 20
    assert plan["control"]["fixed_training_metrics"] == (
        v36.CONTROL_FIXED_TRAINING_METRICS
    )
    assert plan["fixed_candidate"]["weights"] == v36.CANDIDATE_WEIGHTS
    assert plan["decision_order"] == v36.EXPECTED_DECISION_ORDER
    assert plan["ga_authorized"] is False
    assert plan["parameter_search_authorized"] is False
    assert plan["framework_changes"] is False
    assert (
    ...
    original = json.loads(v36.PLAN_PATH.read_text(encoding="utf-8"))
    for index, (section, field, value) in enumerate(mutations):
        ...
        with pytest.raises(ValueError, match="plan SHA256 changed"):
            v36.load_and_validate_plan(path)
        monkeypatch.setattr(v36, "EXPECTED_PLAN_SHA256", v36._sha256(path))
...
def test_source_lock_rejects_missing_extra_and_wrong_identity(
    ...
    with pytest.raises(ValueError, match=message):
        v36.load_and_validate_source_lock(path)


def test_source_lock_missing_file_fails_loudly(tmp_path: Path):
    with pytest.raises(FileNotFoundError):
        v36.load_and_validate_source_lock(tmp_path / "missing_source_lock.json")
...
def test_capture_identity_rejects_validly_formed_but_wrong_source_digest(
    ...
    repo_root = Path(v36.__file__).resolve().parent
    factor_paths = {
        name: repo_root / "factor_db" / "factors" / f"{name}.py"
        for name in v36.BASE_FACTOR_NAMES
    }
    factor_paths[v36.NEW_FACTOR_NAME] = repo_root / v36.NEW_V36_SOURCE_PATHS[
        "factor"
    ]
    filter_path = repo_root / "factor_db" / "factors" / "filter.py"
    factor_classes = [type(name, (), {}) for name in v36.ALL_FACTOR_NAMES]
    filter_classes = [type(name, (), {}) for name in v36.EXPECTED_FILTER_NAMES]
    source_paths = {
        **{
    ...
    monkeypatch.setattr(v36, "_source_path", lambda value: source_paths[value])
    source_digests = {
        name: f"{index + 1:064x}"
        for index, name in enumerate(v36.NEW_V36_SOURCE_PATHS)
    }
    ...
    def fake_identity_entry(path: Path) -> dict[str, str]:
        ...
        try:
            ...
            relative = ""
        if relative in v36.EXPECTED_BASE_SOURCE_SHA256:
            digest = v36.EXPECTED_BASE_SOURCE_SHA256[relative]
        elif relative in v36.EXPECTED_EXECUTION_SOURCE_SHA256:
            digest = v36.EXPECTED_EXECUTION_SOURCE_SHA256[relative]
        else:
            new_name = next(
                (
                    name
                    for name, source_path in v36.NEW_V36_SOURCE_PATHS.items()
            ...
            elif resolved == v36.PLAN_PATH.resolve():
                digest = v36.EXPECTED_PLAN_SHA256
            elif resolved == v36.CANDIDATE_SOURCE_PATH.resolve():
                digest = v36.EXPECTED_CANDIDATE_SOURCE_SHA256
            elif resolved == v36.CONTROL_DAILY_REFERENCE_PATH.resolve():
                digest = v36.EXPECTED_CONTROL_DAILY_REFERENCE_SHA256
            elif resolved == v36.EXPECTED_RUNTIME_PATH.resolve():
                digest = v36.EXPECTED_RUNTIME_SHA256
            else:
                digest = "e" * 64
        return {"path": str(resolved).replace("\\", "/"), "sha256": digest}

    monkeypatch.setattr(v36, "_identity_entry", fake_identity_entry)
    identity = v36._capture_input_identity(
        plan_path=v36.PLAN_PATH,
        source_lock_path=lock_path,
        candidate_source_path=v36.CANDIDATE_SOURCE_PATH,
        control_daily_reference_path=v36.CONTROL_DAILY_REFERENCE_PATH,
        runtime_path=v36.EXPECTED_RUNTIME_PATH,
        script_path=Path(v36.__file__),
        factor_classes=factor_classes,
        filter_classes=filter_classes,
    )
    assert set(identity["new_v36_sources"]) == set(v36.NEW_V36_SOURCE_PATHS)
    assert "independent_new_source_lock" in identity

    lock["sources"]["factor"]["sha256"] = "f" * 64
    lock_path.write_text(json.dumps(lock), encoding="utf-8")
    with pytest.raises(ValueError, match="current source differs"):
        v36._capture_input_identity(
            plan_path=v36.PLAN_PATH,
            source_lock_path=lock_path,
            candidate_source_path=v36.CANDIDATE_SOURCE_PATH,
            control_daily_reference_path=v36.CONTROL_DAILY_REFERENCE_PATH,
...
def test_any_locked_source_change_is_detected_at_every_recheck_stage(
    ...
    identity = {
        "new_v36_sources": {
            "adapter": {
                "path": str(source),
                "sha256": v36._sha256(source),
            }
        }
    }
    v36._assert_input_hashes_unchanged(identity)
...
def test_source_rechecks_are_positioned_before_structure_each_arm_and_manifest():
    run_source = inspect.getsource(v36.run)
    arm_source = inspect.getsource(v36._run_authoritative_arm)
    manifest_source = inspect.getsource(v36._write_and_verify_manifest)
...
def test_each_real_authoritative_arm_calls_once_lightweight_false_without_retry(
    ...
    identity = {
        "role": role,
        "label": v36.CONTROL_LABEL if role == "control" else v36.CANDIDATE_LABEL,
        "individual_config": {
            "weights": copy.deepcopy(
                v36.CONTROL_WEIGHTS if role == "control" else v36.CANDIDATE_WEIGHTS
            ),
            "filter_factors": {
                name: True for name in v36.EXPECTED_FILTER_NAMES
            },
            "stock_pool": list(v36.RANK_PREFIXES),
    ...
    arrays = {
        **{name: np.ones((1, 1), dtype=np.float32) for name in v36.ALL_FACTOR_NAMES},
        **{
        ...
        **{
            ...
            for name in v36.EXPECTED_FILTER_NAMES
        },
    }
    monkeypatch.setattr(
        v36,
    ...
    monkeypatch.setattr(v36, "_backtest_direct", stop_backtest)
    sequence = list(initial_sequence)
    with pytest.raises(StopAfterCall):
        v36._run_authoritative_arm(
            identity=identity,
            output_dir=tmp_path,
            data={"stock_codes": np.asarray(["000001.SZ"])},
            arrays=arrays,
            score_keys=set(v36.ALL_FACTOR_NAMES),
...
def test_training_entrypoint_has_no_amendment_old_factor_search_or_stage_path():
    module_source = Path(v36.__file__).read_text(encoding="utf-8")
    ...
    assert "CompletedPriorMonthTurnoverStrict" not in imported_modules
    assert "_run_ga" not in called_names
    assert "subprocess" not in imported_modules
    assert "multiprocessing" not in imported_modules
    assert list(inspect.signature(v36.run).parameters) == ["output_dir"]
    main_source = inspect.getsource(v36.main)
...
def test_module_has_no_hidden_old_factor_ga_search_resume_or_holdout_executor():
    ...
    assert "research_v35_prior_month_turnover_ablation" not in module_source
    run_source = inspect.getsource(v36.run)
    for holdout_date in (
        "2019-01-01",
        "2019-01-02",
...
def test_new_factor_loader_is_exactly_hist20_and_not_a_profile_mutation():
    factor_class = v36._new_factor_class()
    assert factor_class.__name__ == v36.NEW_FACTOR_NAME
    ...
    source = inspect.getsource(v36._new_factor_class)
    assert "CompletedSignedAmountImbalance20Strict" in source
    assert "get_profile" not in source


def test_full_panel_v36_amihud_one_bit_mismatch_aborts_before_any_arm():
    arrays, score_keys, candidates, date_indices, pool_columns = (
        _structural_fixture()
    )
    arrays[f"_factor_valid_{v36.NEW_FACTOR_NAME}"][-1, -1] = False
    arm_calls = []
    with pytest.raises(ValueError, match="finite mask differs from Amihud"):
        v36._structural_isolation_audit(
            arrays=arrays,
...
@pytest.mark.parametrize(
    "mask_name",
    ["_active_factor_intersection", *v36.EXPECTED_FILTER_NAMES],
)
def test_each_training_pool_mask_one_bit_mismatch_aborts_at_zero_calls(
    ...
    shape = arrays[f"_factor_valid_{v36.NEW_FACTOR_NAME}"].shape
    ...
    def fake_masks(_arrays, _score_keys, config):
        ...
        if v36.NEW_FACTOR_NAME in config["weights"]:
            masks[mask_name][0, 0] = False
        return masks

    monkeypatch.setattr(v36, "candidate_filter_masks", fake_masks)
    arm_calls = []
...
def test_defensive_composite_and_count_mismatch_branches_abort_at_zero_calls(
    ...
    monkeypatch.setattr(v36.np, "array_equal", targeted_array_equal)
    arm_calls = []
    with pytest.raises(ValueError, match=message):
        v36._structural_isolation_audit(
            arrays=arrays,
...
def test_every_control_canary_metric_obeys_the_absolute_tolerance_boundary(
    ...
    row = {
        "role": "control",
        "label": v36.CONTROL_LABEL,
        **copy.deepcopy(v36.CONTROL_CANARY),
    }
    ...
    if metric_path == "minimum_daily_selectable_candidates":
        v36.validate_control_canary(row)
        _set_metric(row, metric_path, int(expected) + 1)
    else:
        edge = float(expected) + v36.CANARY_ABSOLUTE_TOLERANCE
...
def test_control_reference_uses_np_array_equal_for_all_four_arrays(
    ...
    dates = [v36.TRAIN_FIRST_DATE, v36.TRAIN_LAST_DATE]
    ...
    audit = v36._validate_control_daily_reference(
        reference=reference,
        dates=dates,
    ...
    with pytest.raises(ValueError, match=f"pinned V35 {field}"):
        v36._validate_control_daily_reference(
            reference=reference,
            dates=dates,
            daily_returns=returns,
            daily_exposures=exposures,
...
def test_default_pinned_v35_npz_has_seven_fields_but_loader_returns_control_only():
    assert v36._sha256(v36.CONTROL_DAILY_REFERENCE_PATH) == (
        v36.EXPECTED_CONTROL_DAILY_REFERENCE_SHA256
    )
    with np.load(v36.CONTROL_DAILY_REFERENCE_PATH, allow_pickle=False) as saved:
        assert set(saved.files) == _v35_npz_fields()
    reference = v36._load_control_daily_reference()
    assert set(reference) == _v35_control_fields()
    assert all(
        value.shape == (v36.EXPECTED_TRAIN_DAYS,)
        for value in reference.values()
    )
    assert reference["dates"].dtype.kind == "U"
    assert str(reference["dates"][0]) == v36.TRAIN_FIRST_DATE
    assert str(reference["dates"][-1]) == v36.TRAIN_LAST_DATE
...
def test_synthetic_exact_seven_field_v35_npz_returns_only_control_arrays(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    path = tmp_path / "seven_fields.npz"
    arrays = _synthetic_v35_npz_arrays()
    _write_synthetic_v35_npz(path, arrays)
    monkeypatch.setattr(
        v36, "EXPECTED_CONTROL_DAILY_REFERENCE_SHA256", v36._sha256(path)
    )
    reference = v36._load_control_daily_reference(path)
    assert set(reference) == _v35_control_fields()
    for name in _v35_control_fields():
        np.testing.assert_array_equal(reference[name], arrays[name])


@pytest.mark.parametrize("field_change", ["missing", "extra"])
def test_v35_npz_missing_or_extra_field_fails_loudly(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    field_change: str,
):
    ...
    if field_change == "missing":
        ...
        arrays["unexpected_candidate_metric"] = np.zeros(
            v36.EXPECTED_TRAIN_DAYS, dtype=np.float64
        )
    ...
    with pytest.raises(ValueError, match="reference fields changed"):
        v36._load_control_daily_reference(path)
...
def test_candidate_reference_negative_or_nonintegral_count_fails_loudly(
    ...
    for label, values, message in (
        (
            "negative",
            np.full(v36.EXPECTED_TRAIN_DAYS, -1, dtype=np.int64),
            "contains a negative candidate_daily_selectable_candidates",
        ),
        (
            "float",
            np.full(v36.EXPECTED_TRAIN_DAYS, 137.0, dtype=np.float64),
...
def test_primary_gate_boundaries_distinguish_at_least_from_strictly_exceeds():
    control = _gate_row(role="control", label=v36.CONTROL_LABEL)
    candidate = _gate_row(role="candidate", label=v36.CANDIDATE_LABEL)
    ...
    checks = v36.primary_gate_checks(candidate, control)
    ...
    assert all(v36.primary_gate_checks(candidate, control).values())
...
def test_strict_json_rejects_every_nonfinite_constant(constant: str):
    with pytest.raises(ValueError, match="forbidden constant"):
        v36._loads_strict_json(f'{{"nested": [1, {constant}]}}')
...
def test_machine_trade_audit_rejects_every_exactness_and_identity_drift(
    ...
    baseline = v36._audit_trade_log_exact_open(
        role="control",
        label=v36.CONTROL_LABEL,
    ...
    failed = v36._audit_trade_log_exact_open(
        role="candidate",
        label=v36.CANDIDATE_LABEL,
...
def test_rendered_trade_audit_compares_all_sort_cells_in_original_order(
    ...
    table = v36.single_report_module._make_trade_table(trades, names)
    report_path = tmp_path / "single_report.html"
    _write_report_data(report_path, {"tables": {"trades": table}})
    passed = v36._audit_rendered_trade_table_exact_open(
    ...
    failed = v36._audit_rendered_trade_table_exact_open(
        report_path=report_path,
        trade_log=trades,
        stock_name_map=names,
        open_price=np.asarray([[10.0]], dtype=np.float64),
...
def test_rendered_trade_audit_rejects_row_reordering(tmp_path: Path):
    ...
    table = v36.single_report_module._make_trade_table(trades, {})
    table["rows"].reverse()
    report_path = tmp_path / "single_report.html"
    _write_report_data(report_path, {"tables": {"trades": table}})
    failed = v36._audit_rendered_trade_table_exact_open(
...
def test_kline_audit_rejects_code_date_event_episode_order_and_column_drift(
    ...
    with pytest.raises(ValueError, match=message):
        v36._audit_html_kline_training_bounds(path, {"000001.SZ"})
...
def test_kline_report_decodes_and_audits_exact_training_bounds(tmp_path: Path):
    ...
    payload = _valid_kline_payload()
    _write_kline_report(path, payload)
    assert v36._decode_html_kline_payload(path) == payload
    audit = v36._audit_html_kline_training_bounds(path, {"000001.SZ"})
    assert audit["passes"] is True
    assert audit["actual_first_date"] == v36.TRAIN_FIRST_DATE
    assert audit["actual_last_date"] == v36.TRAIN_LAST_DATE
...
def test_html_and_kline_decoders_reject_duplicate_nonfinite_and_invalid_base64(
    ...
    with pytest.raises(ValueError, match="payload count is invalid"):
        v36._decode_html_report_data(path)
    ...
    with pytest.raises(ValueError):
        v36._decode_html_kline_payload(path)
...
def _create_arm_artifacts(output_dir: Path, role: str, label: str) -> dict:
    ...
    trade = _trade_row()
    names = {"000001.SZ": "平安银行"}
    table = v36.single_report_module._make_trade_table([trade], names)
    report_data = {
        "tables": {"trades": table},
    ...
    for name, payload in (
        ...
        v36._write_json(run_dir / name, payload)
    (run_dir / "single.log").write_text(
        "synthetic V36 arm completed successfully\n", encoding="utf-8"
    )
    return {
        "role": role,
        "label": label,
...
def _create_both_arm_artifacts(output_dir: Path) -> list[dict]:
    return [
        _create_arm_artifacts(output_dir, "control", v36.CONTROL_LABEL),
        _create_arm_artifacts(output_dir, "candidate", v36.CANDIDATE_LABEL),
    ]
...
def test_both_arm_report_kline_json_log_and_date_obligations_parse(tmp_path: Path):
    ...
    v36._validate_report_obligations(tmp_path, summaries)
...
def test_report_obligations_fail_closed_on_missing_unparseable_or_bad_dates(
    ...
    summaries = _create_both_arm_artifacts(tmp_path)
    run_dir = tmp_path / v36.CANDIDATE_LABEL
    if corruption == "missing":
        ...
        record = v36._load_strict_json(run_dir / "record.json")
        record["dates"][-1] = "2018-12-27"
        v36._write_json(run_dir / "record.json", record)
...
def test_manifest_rehash_detects_any_artifact_write_during_commit(
    ...
    identity = {
        "source": {"path": str(source), "sha256": v36._sha256(source)}
    }
    original_write = v36._write_json
    ...
    monkeypatch.setattr(v36, "_write_json", write_then_mutate)
    with pytest.raises(RuntimeError, match="artifact hashes failed revalidation"):
        v36._write_and_verify_manifest(
            output_dir=output,
            status="synthetic",
...
def test_rolling_contract_uses_756_63_final_start_and_linear_p10():
    ...
    rolling = v36._rolling_blocks(dates, daily)
...
def test_concentration_uses_calendar_log_wealth_and_nonpositive_default():
    ...
    audit = v36._return_concentration(dates, daily)
    ...
    losing = v36._return_concentration(dates, np.full(4, -1.0))
...
def test_secondary_is_fixed_5bps_and_freeze_requires_every_secondary_gate(
    ...
    dates = _fixed_date_strings()
    daily = np.zeros(v36.EXPECTED_TRAIN_DAYS, dtype=np.float64)
    calls = []
    monkeypatch.setattr(
        v36,
        "_rolling_blocks",
        lambda *_args: {
            "p10_calmar": v36.SECONDARY_AUDITS["rolling_3y_p10_calmar_min"]
        },
    )
    monkeypatch.setattr(
        v36,
        ...
        lambda *_args: {
            "largest_year_log_wealth_share": v36.SECONDARY_AUDITS[
    ...
    monkeypatch.setattr(v36, "stress_daily_returns", fake_stress)
    monkeypatch.setattr(
        v36, "natural_calendar_metrics", lambda *_args: _fake_natural_metrics()
    )
    audit, rebalance, stressed = v36._secondary_audits(
        dates=dates,
        candidate_daily_returns=daily,
        candidate_report_path=Path("synthetic_report.html"),
    )
    assert calls == [(v36.EXPECTED_TRAIN_DAYS, 1.0, 5.0)]
    ...
    primary_rows, primary = v36.annotate_primary_decision([control, candidate])
    frozen_rows, frozen = v36.finalize_training_decision(
    ...
    rejected_rows, rejected = v36.finalize_training_decision(
...
def test_secondary_is_forbidden_after_any_primary_failure():
    ...
    rows, primary = v36.annotate_primary_decision([control, candidate])
    assert primary["secondary_audit_allowed"] is False
    unchanged_rows, unchanged = v36.finalize_training_decision(
        rows, primary, None
    )
    ...
    with pytest.raises(ValueError, match="secondary audit ran after a primary failure"):
        v36.finalize_training_decision(
...
def _install_synthetic_run(
    ...
    *,
    ...
    identities = [
        {
            ...
            "role": "control",
            "label": v36.CONTROL_LABEL,
            "source_candidate_index": v36.CONTROL_SOURCE_INDEX,
            "change_from_control": {},
            "individual_config": control_config,
        },
        {
            ...
            "label": v36.CANDIDATE_LABEL,
            "source_candidate_index": v36.CONTROL_SOURCE_INDEX,
            "change_from_control": {v36.NEW_FACTOR_NAME: 0.1},
            "individual_config": candidate_config,
        },
    ]
    control_row = {
    ...
    script_hash = v36._sha256(Path(v36.__file__))
    identity_hashes = {
        "authoritative_script": {
            "path": str(Path(v36.__file__).resolve()),
            "sha256": script_hash,
        },
        "synthetic_test_identity": "no_real_data_loaded",
    }
    monkeypatch.setattr(v36, "load_and_validate_plan", lambda _path: {})
    monkeypatch.setattr(
        v36, "_factor_classes", lambda: ([DummyFactor], [DummyFactor])
    )
    monkeypatch.setattr(
        v36, "_unambiguous_runtime_path", lambda: v36.EXPECTED_RUNTIME_PATH
    )
    monkeypatch.setattr(
        v36, "_capture_input_identity", lambda **_kwargs: identity_hashes
    )
    monkeypatch.setattr(v36, "load_control_source", lambda _path: {})
    monkeypatch.setattr(
        v36,
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ({}, set(v36.ALL_FACTOR_NAMES)),
    )
    monkeypatch.setattr(v36, "_verify_candidate_validity_semantics", lambda *_: None)
    monkeypatch.setattr(v36, "_compute_list_dates", lambda *_: {})
    monkeypatch.setattr(v36, "_assert_input_hashes_unchanged", lambda _identity: None)
    monkeypatch.setattr(
        v36,
    ...
    monkeypatch.setattr(
        v36, "_load_control_daily_reference", lambda: copy.deepcopy(reference)
    )
    ...
    monkeypatch.setattr(v36, "_run_authoritative_arm", fake_arm)
    monkeypatch.setattr(v36, "_validate_report_obligations", lambda *_args, **_kwargs: None)
    ...
    monkeypatch.setattr(v36, "_secondary_audits", fake_secondary)
    monkeypatch.setattr(
        v36,
        "_build_summary",
        lambda **kwargs: {
    ...
    monkeypatch.setattr(
        ...
        lambda path, *_args: v36._write_json(path, {"configuration_frozen": True}),
    )
    return state
...
def test_synthetic_run_has_exact_fail_closed_call_order_and_manifests(
    ...
    if failure_stage == "structural":
        ...
        return
    manifest = v36._load_strict_json(output / "manifest.json")
    ...
    assert manifest["generated_artifact_sha256"] == v36._generated_file_hashes(
        output
    )
    assert not (output / v36.CANDIDATE_LABEL).exists() == (
...
def test_existing_output_is_rejected_before_any_input_read(
    ...
    reads = []
    monkeypatch.setattr(
        v36, "load_and_validate_plan", lambda *_args: reads.append("plan")
    )
