...
TODAY = date.today()
DOWNLOAD_TRADING_DAYS = 10


def _download_kline_all():
    """下载所有股票最近 ~10 交易日 K 线，写 parquet 并返回内存数据。
    ...
    logger.info("[K线] 已有 %d 只, 新下载 %d 只", len(existing), len(new_stocks))

    fetch_offset = DOWNLOAD_TRADING_DAYS + 20
    now = datetime.now()
    ...
    def _process_batch(batch_codes):
        ...
        for code in batch_codes:
            ...
            try:
                ...
                df_new.sort_values('time', ascending=False, inplace=True)
    ...
    logger.info("[K线] 处理: %d 只, %d 批", len(existing_codes), len(batches))
    t1 = time.time()
    total_updated = 0
    ...
    if new_stocks:
        ...
        for code in new_stocks:
            try:
                ...
                logger.warning("[K线] 新股票 %s 下载失败: %s", code, e)
        logger.info("[K线] 新股票完成: %d/%d 只 (%.0fs)", new_ok, len(new_stocks), time.time() - t2)
...
def _patch_npz_incremental(kline_data: dict):
    ...
    if not npz_files:
        logger.info("[NPZ增量] 无现有 NPZ，执行全量构建")
    ...
    n_old = len(old_dates)
    logger.info("[NPZ增量] 已加载: %d 日 x %d 股 (%.0fs)", n_old, len(stock_codes), time.time() - t0)
    ...
    logger.info("[NPZ增量] 新增交易日: %d 个 (已有 %d 个)",
    ...
    logger.info("[NPZ增量] 覆盖 %d 个 (date,stock) 单元格 (%.0fs)",
    ...
    logger.info("[NPZ增量] 保存: %s (%.1f MB, %.0fs)", output_path.name, file_size_mb, elapsed)
    return output_path


def update_live_quick():
    t0 = time.time()
    logger.info("=" * 60)
    logger.info("开盘快速更新: K线增量 → 增量修补 NPZ")
    logger.info("=" * 60)

    # Phase 1: 下载 K 线 + 写 parquet + 收集内存数据
    logger.info("--- Phase 1: K线下载 ---")
    kline_data = _download_kline_all()

    # Phase 2: 增量修补 NPZ（只改 K 线字段，不动财务/股本/ST）
    logger.info("--- Phase 2: 增量修补 NPZ ---")
    _patch_npz_incremental(kline_data)

    logger.info("=" * 60)
    logger.info("开盘更新完成! 总耗时 %.0fs", time.time() - t0)
    logger.info("=" * 60)
