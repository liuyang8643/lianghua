...
class TopN:
    ...
    def __init__(
        ...
        core_logger.debug(
            f"TopN 计算 {format_qmt_date(self.base_date)} 因子分数，"
            f"共计{len(self.stock_list)}只股票，{len(self.factors)}个因子"
        )
        ...
        core_logger.debug(
            f"TopN 计算 {format_qmt_date(self.base_date)} 因子分数完成，"
            f"有效股票数: {self._count_valid_stocks()}"
        )
