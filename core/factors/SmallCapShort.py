...
class SmallCapShort(BaseFactor):
  """
  Short-cycle SmallCap composite:
  - Small-cap anchor (liquidity/size proxy).
  - Retail participation level (cross-sectional crowding proxy).
  - Retail flow momentum (short-cycle reversal proxy).
  """
  ...
  def _calc_retail_ratio_and_momentum(self, ctx: FactorCtx, history_data) -> tuple[float, float]:
    ...
    if abs(ma_ratio) <= 1e-8:
      ...
      # Same sign convention as RetailFlowMomentum: retail retreat -> positive score.
