...
评分设计:
- base_score: ADX趋势强度 × DI方向 (连续值，不加限制)
...
class ADXFactor(BaseFactor):
    ...
    评分逻辑:
    1. base_score: ADX趋势强度 × DI方向分数（不加限制）
    ...
    def calc(self, ctx: FactorCtx) -> FactorResult:
        ...
        # ADX趋势强度 × 方向分数，连续分数不加限制
        adx_strength = adx / 100.0
        base_score = adx_strength * direction_normalized

        # === 2. 信号加成 (50%) ===
        signal_bonus = 0.0

        # +DI上穿-DI金叉检测
        required_days = self.cross_valid_days + self.adx_period * 2
        ...
        if len(valid_plus_di) < self.cross_valid_days + 1:
            raise ValueError(f"有效DI数据不足: 需要{self.cross_valid_days + 1}个点，实际{len(valid_plus_di)}个")
