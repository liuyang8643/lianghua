...
E4（v19、新默认、seed 20260921、4000 轮 / 512 万 transitions）：训练 Calmar 峰值 1.430（3000 轮）、终值 1.344、最后 5 点 1.311±0.025，在 1.30～1.43 平台，未达 GA 1.50；验证 0.55～0.71、测试 0.57～0.75；std 收缩至 0.10，末段权重跨日期 std 升至 0.03～0.04（状态路径开始微弱起作用）。收敛后的静态权重与 GA 冠军方向一致但 Reversal/BiliA 明显偏低——在 γ=0.99、λ=0.95、n_steps=64 下 GAE 有效信用窗口约 17～30 日，长周期兑现的因子被系统性低估，这是与 GA 全周期配对评估的结构差异。据此新增 CLI `--gae-lambda`（默认仍 0.95，写入 identity），E5 以 v20 schema + λ=0.98 + n_steps=128（batch 640）+ 2500 轮检验信用窗口假设。E5 结果（640 万 transitions）：训练 Calmar 峰值 1.358、终值 1.305、最后 5 点 1.326±0.020，与 E4 平台相当而非更高，Reversal/BiliA 权重未上升——信用窗口假设不获支持；但 critic explained_variance 由≈0 升至 0.32～0.50，权重跨日期 std 升至 0.05～0.07（状态路径开始起作用），validation 0.53～0.79。E6 为 E5 的 verified resume（同冻结源码、同契约）：再跑 1000 轮仍停在 1.24～1.37 平台、权重冻结，已手动停止。E4/E5 平台出现时 Gaussian std 均已收缩到 0.10～0.13，20 个环境的相对差异被个股噪声淹没、梯度消失，是平台的候选解释；据此新增 CLI `--ent-coef`（默认 0，写入 identity），E7（ent_coef 0.02）在 900 轮内 std 由 0.20 膨胀到 1.35、权重全部饱和到端点、Calmar 回到 1.0～1.2 震荡，说明 0.02 远大于策略梯度对 log_std 的推力，已终止；E8 改用 ent_coef 0.003 复测（2500 轮 / 320 万 transitions）：std 稳定在 0.21～0.26，训练 Calmar 峰值 1.390、最后 5 点 1.285±0.049，仍是同一平台，"std 塌缩"假设不成立；权重跨日期 std 升到 0.17～0.22（策略首次明显状态相关）但未转化为更高训练 Calmar，验证降至 0.43～0.77。E4/E5/E6/E8 在 λ、n_steps、std 三个维度上都收敛到 1.3～1.43 平台，该平台是当前 Reward（年化增量 − 回撤增量、H/252 缩放）与逐步信用分配结构下 PPO 的稳定最优点；突破它需改变奖励形状（如逐日 log 收益替代年化增量）或状态路径的学习信号，均属新 schema/新 root 的独立决策，尚未执行。ent_coef 与 gae_lambda 默认值保持 0 / 0.95 不变。

## 38. 十二因子（修正版 Amihud）与固定 20 只持仓（2026-09-22）

用户授权：GA/PPO 生产词表追加修正版 Amihud 非流动性，固定持仓由 50 改为 20，并以此前实验验证的最优参数启动 10 万轮 PPO。

- `CompletedAmihudIlliquidity20`（factor/library/styles.py）：完整 [T-20, T) 窗口内 `|官方简单收益| / (amount/1e8)` 的均值。旧实现（factor_db/AmihudIlliquidity，仍用于 WBR-live）对 `amount_known/1e8` 无有效性门控：成交额为 0 或极小（停牌日、单笔成交）产生 inf 或高出截面多个量级的值，且 `np.cumsum` 把 inf 传播到之后所有窗口。修正：仅当官方收益与成交额有限且 `amount ≥ 1e5` 元时该日有效，20 天中至少 15 天有效，输出窗口均值；下游评分与 actor 只消费截面 rank。真实 2015 年切片：成员覆盖 85%，中位 0.011、99 分位 0.073、最大 27.7，无 inf。
- 词表为原 11 因子 + `CompletedAmihudIlliquidity20`（末位），factor schema v10；ActionSchema v21 `day-config-v21-amihud12-hold20`（13 维 Box，`FIXED_BUY_N=20`，`single_buy_pct=1/20`，换股仍 [0.05,0.2] 即每日检查 floor(20×rate)=1～4 只最差持仓）；Observation v22、encoder v22、normalizer v14（因子排名 12 列，PolicyHistory 每行 16 维）、environment v39、bundle v47、journal v10、PPO identity v91；`configs/config.json`、`configs/strategy.yaml` 同步（新因子静态权重 0，buy_n 20）。旧 v20 checkpoint/GA 种群/normalizer 均不可续入。
- 10 万轮训练 `ppo11_f1_amihud12_hold20_100k_20260922` 使用当前默认：log_std_init −1.6、行均值 advantage 基线、lr 3e-3、n_epochs 1、target_kl 0.03、20 环境 × 64 步、batch 640、eval 每 200 轮、Reward v5、seed 20260922。未启用 gae_lambda/ent_coef/动作头增益/对偶探索等探针参数（E5～E16 均未证明有效；E16 中途终止，无结论）。
- 既有无关失败测试：`test_completed_6030_pool_beta60_strict`/`test_completed_official_reversal_low_vol12020_strict` 的"无逐股循环"源码扫描在基线上即失败（扫描对象含 Numba 核的 `for stock` 循环），未随本次修改。

## 37. Reward v5：逐日年化 log 收益增量（2026-09-21）

用户明确当前只关心训练集 Calmar 上限，并授权由 agent 决定优化方向；选择先改奖励形状。v4 的每日回报项是"以固定 H 年化的算术收益增量" `expm1(252·c_t/H) − expm1(252·c_{t−1}/H)`，等价于 `exp(252·c_{t−1}/H)·Δlog`——同样的日 log 收益在 episode 后段（账户已复利 1.7 倍）比前段权重高 1.7 倍，critic 目标随 episode 进度非平稳，且回报/回撤梯度比为 1:1，低于训练最优点处 Calmar 梯度比（≈1:1.45）。v5 `horizon-balanced-log-return-incremental-max-drawdown-v5` 把回报项改为 `net_log_return·252/H`，reward = `net_log_return − (H/252)·ΔMDD`，整段和恒等于 `H/252·(log1p(年化收益) − 最大回撤)`：每一天每单位 log 收益权重相同，回撤相对权重 ≈1:1.7，更接近 Calmar 梯度方向。`EpisodeReward.annualized_return_increment` 更名为 `annualized_log_return_increment`，诊断字段同步；`MAX_DRAWDOWN_PENALTY_WEIGHT` 仍为 1，H/252 缩放不变。environment v38、bundle v46、PPO identity v90；旧 v4 奖励 checkpoint 不可续训。此前对"年化增量在 episode 前段被放大"的口头判断有误（v4 在固定 H 下并无小 t 放大），已在此更正；脉冲周期性来自 20 环境同步 episode、日历事件与 v4 的复利权重。E9 以 v5 + v20 schema + 新默认、seed 20260921、2500 轮与 E4/E8 对照。E9 结果：训练 Calmar 峰值 1.358、最后 5 点 1.273±0.013，同轮次与 E4/E8 持平，critic EV 仍≈0，验证中段最高 0.98 后回落至 0.6～0.77。Reward v5 未加速训练集收敛、未抬高平台；v4→v5 仅是形式上更平稳、更贴近 Calmar 梯度的等价表述，保留为当前版本但不宣称收益改善。至此 std 初值/基线/学习率/λ/n_steps/ent_coef/换手下限/奖励形状八个维度全部收敛到同一 1.3～1.43 训练平台，静态策略上限≈GA 1.50；进一步提升训练集 Calmar 只剩状态路径（网络/优化器结构）方向，需用户授权。

信噪比探针（2026-09-21，用户确认后执行）：以每次更新 1280 条样本中"12 维动作对 advantage 的线性 R²"及逐维相关的跨更新符号一致性为读数。基线 E4/E8/E9 的 R² 为 0.0099～0.0114，噪声期望 12/1280=0.0094（比值 1.05～1.22），一致性 0.14～0.22（≈噪声），即每步策略梯度在噪声底线，学习仅靠数千次更新累加一致分量，状态相关信号无法分辨。P1（γ0.9、λ0.8，200 轮）比值 0.91、一致性 0.28：缩短信用窗口无效。P2（40 个同步环境，200 轮）比值 1.35、一致性 0.53：与配对样本翻倍带来的 √2 方差下降一致，是样本量效应而非每样本信号提升。新增 `--gamma` CLI（默认 0.99）。结论：日频回报下每样本可提取的动作信号固定，PPO 只能通过样本量逼近静态最优（≈GA 1.50），训练集"远超 GA"在当前动作/回报结构下不可达；后续若坚持训练集目标，需改变问题结构（更高信息量的动作空间或状态信息），而非继续调参。

用户随后授权 agent 不再询问、持续自主做快速实验并记录，仅在有进展时反馈。既然每样本信号固定，可控杠杆是单位墙钟的独立样本数：实测 learn 阶段（3 epochs、GPU 反传）占每轮墙钟 70%～80%（E4 2.06 s/2.55 s，E9 3.38 s/4.17 s），40 环境每轮样本翻倍而 learn 仅 +30%。E10 = 40 环境 × 64 步（2560/轮）+ n_epochs 1（4 个 minibatch/轮）+ 其余新默认，2000 轮≈512 万 transitions（与 E4 等样本量），以等墙钟/等样本的训练 Calmar 与 E4/E9 对比；判据为到达 1.3 平台的墙钟是否缩短，以及是否越过 1.43。

吞吐实验暴露出内存上限：机器空闲提交内存约 39GB，训练主进程 15～28GB、20 个 worker 各 0.7～1GB、三段共享数组 16GB，40 环境时 worker 在导入 Torch 时触发 `WinError 1455`（页面文件太小），30/16/20 环境也在验证 split 因子预计算处 `MemoryError`。两项工程修复：(1) 验证/测试 split 在 rollout worker 创建前预先准备并常驻（`EVALUATION_CACHE_PROTOCOL.prepared_splits = eager_once_before_rollout_workers_shared_readonly_until_exit`），避免 worker 存活时的瞬时峰值；(2) 新增无 Torch 的 `ai/rl/rollout_worker.py`（自实现 SB3 worker 命令循环、cloudpickle 包装、`RolloutEnvFactory`、`WorkerWBRGymEnv`、seed 分配），`ai/rl/__init__.py` 改为懒加载 `RLPolicy`，spawn 子进程导入链不再含 torch/SB3。E12（20env、n_epochs 1、4000 轮）在修复后启动：提交峰值 89GB（修复前 97GB），每轮约 2.3 s（3 epochs 时 4.2 s）。E10 系列失败目录保留为审计。E12 结果（1370 轮 / 57 分钟停止）：吞吐 510 transitions/s（E9 307），但 learn 阶段固定开销约 2 s/轮与 epoch 数基本无关；单 epoch 下 `approx_kl≈0.001`、`clip≈0.005`，每轮仅 2 次 Adam，策略更新过于保守，换股钉在 0.05 下限，训练 Calmar 峰值 0.92、1200 轮 0.76，等墙钟劣于 E9（约 1.1）。E13（单 epoch + lr 3e-3、1500 轮 / 84 分钟）：250 轮 1.145、500 轮 1.279、750 轮 1.309，约 45 分钟到达 1.3 平台（E4/E9 需 1.8～2.5 小时），末段 1.23～1.29；`approx_kl` 0.004、`clip` 0.05～0.06、std 0.20→0.17。据此 CLI 默认改为 `DEFAULT_N_EPOCHS=1`、`DEFAULT_LEARNING_RATE=3e-3`（target_kl 0.03 不变）。平台高度不变（1.3～1.4）。新增 `--action-head-gain`（默认 0.01 = SB3；TypedActorCriticPolicy 构造参数并写入 checkpoint/identity），E14（gain 1.0，500 轮后停止）：初始策略跨状态 std 0.29，但首批更新 `approx_kl` 0.57 触发 target_kl 早停、权重迅速饱和到端点、换股钉在 0.05 下限，500 轮 Calmar 0.78（E13 同期 1.28），gain 1.0 过大导致失稳；E15（gain 0.1，1250 轮）：峰值 1.327、末值 1.278，与 E13 同平台，权重跨状态 std 仍≈0.004；动作头增益方向关闭，默认保持 0.01。新增 `--antithetic-exploration`（TypedActorCriticPolicy.forward 在采样时对成对同步环境施加 ±ε 镜像噪声；每个环境的动作仍是自身 N(mean,std) 的精确样本，log-prob 与 clipped surrogate 不变，仅降低行均值中心化 advantage 的方差），E16 以新默认 + 对偶探索复测 R² 与平台。静态策略上限约等于 GA 1.50（GA 冠军年化 0.749/回撤 0.500，PPO 平台 0.73/0.51），奖励改形只能缩小差距与加速；超越需动态策略真正贡献。当前结论：新默认已把训练从随机游走变为稳定收敛（1.3～1.4 平台），但尚未达到 GA 训练 1.50，更未超越；GA 1.50 对应验证 0.51（过拟合），PPO 验证最高 0.79～0.91 已高于 GA 与静态 0.61。
