...
用户明确当前只关心训练集 Calmar 上限，并授权由 agent 决定优化方向；选择先改奖励形状。v4 的每日回报项是"以固定 H 年化的算术收益增量" `expm1(252·c_t/H) − expm1(252·c_{t−1}/H)`，等价于 `exp(252·c_{t−1}/H)·Δlog`——同样的日 log 收益在 episode 后段（账户已复利 1.7 倍）比前段权重高 1.7 倍，critic 目标随 episode 进度非平稳，且回报/回撤梯度比为 1:1，低于训练最优点处 Calmar 梯度比（≈1:1.45）。v5 `horizon-balanced-log-return-incremental-max-drawdown-v5` 把回报项改为 `net_log_return·252/H`，reward = `net_log_return − (H/252)·ΔMDD`，整段和恒等于 `H/252·(log1p(年化收益) − 最大回撤)`：每一天每单位 log 收益权重相同，回撤相对权重 ≈1:1.7，更接近 Calmar 梯度方向。`EpisodeReward.annualized_return_increment` 更名为 `annualized_log_return_increment`，诊断字段同步；`MAX_DRAWDOWN_PENALTY_WEIGHT` 仍为 1，H/252 缩放不变。environment v38、bundle v46、PPO identity v90；旧 v4 奖励 checkpoint 不可续训。此前对"年化增量在 episode 前段被放大"的口头判断有误（v4 在固定 H 下并无小 t 放大），已在此更正；脉冲周期性来自 20 环境同步 episode、日历事件与 v4 的复利权重。E9 以 v5 + v20 schema + 新默认、seed 20260921、2500 轮与 E4/E8 对照。静态策略上限约等于 GA 1.50（GA 冠军年化 0.749/回撤 0.500，PPO 平台 0.73/0.51），奖励改形只能缩小差距与加速；超越需动态策略真正贡献。当前结论：新默认已把训练从随机游走变为稳定收敛（1.3～1.4 平台），但尚未达到 GA 训练 1.50，更未超越；GA 1.50 对应验证 0.51（过拟合），PPO 验证最高 0.79～0.91 已高于 GA 与静态 0.61。
