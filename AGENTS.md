...
- Shared project skill `git-session-commit` defines the default commit workflow: clear the staging area first, stage only files involved in the current session, use a Chinese commit summary, prefer rebasing non-mainline branches onto their latest upstream, use merge when integrating work into the mainline branch, and escalate unresolved judgment-heavy conflicts to the user.
